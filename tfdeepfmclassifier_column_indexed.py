...
"""
File: tfdeepfmclassifier_column_indexed.py
...
class TFDeepFMClassifier(object):
    ...
    def _construct_weights(self):
        ...
        proj_node_num = self.field_num + self.factor_num + self.deep_layer_nodes[-1] # bias + wx + non-reduced interaction + DNN
    ...
    def _forward_pass(self):
        ...
        # deep term
        deep_h = tf.reshape(embedding_t, shape=[-1, self.field_num * self.factor_num])
        for i in range(len(self.deep_w)):
        ...
        # joint projection
        h = tf.matmul(tf.concat([linear_h, interaction_h, deep_h], axis=1, name='concat_wd'), self.proj_w, name='final_proj')
